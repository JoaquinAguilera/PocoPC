...
static const char *const luaX_tokens [] = {
    ...
    "+=", "-=", "*=", "/=", "%=",
    "<<", ">>", "::", "<eof>",
    "<number>", "<integer>", "<name>", "<string>", "<eol>"
};
...
static int read_numeral (LexState *ls, SemInfo *seminfo) {
  ...
  for (;;) {
    ...
    else if ((lisxdigit(ls->current) && !lislalpha(ls->current)) || ls->current == '.')  /* '%x|%.' */
      save_and_next(ls);
    else break;
  }
  //if (lislalpha(ls->current))  /* is numeral touching a letter? */
    //save_and_next(ls);  /* force an error */ except for don't because it's ok
  save(ls, '\0');
...
static int llex (LexState *ls, SemInfo *seminfo) {
  ...
  for (;;) {
    switch (ls->current) {
      case '\n': case '\r': {  /* line breaks */
        inclinenumber(ls);
        if (ls->checkeol) { ls->checkeol = 0; return TK_EOL; }
        break;
      }
      ...
      case '-': {  /* '-' or '--' (comment) */
        next(ls);
        if (check_next1(ls, '=')) return TK_SUBE;  /* '//' */
